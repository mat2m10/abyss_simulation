...
AF_prob = pd.read_pickle(f"../data/estimated p values/estimated_AF_G{G}_L{L}_c{c}_k{k}_M{M}.pkl")
...
for i in range(iterations):    
    ...
    for snp in list(complete.columns):
        ...
        try:
            ...
            snps_abyss.append(snp)
            Ps_abyss.append(p_values[1])
        except Exception as e:
            print(e)
    

    df_abyss = pd.DataFrame(data={'snps': snps_abyss, 'P_vals_abyss': Ps_abyss})
    log10_p_abyss = np.sort(-np.log10(df_abyss['P_vals_abyss']))
    df_abyss['log10_p_abyss_sorted'] = log10_p_abyss
    dfs.append(df_abyss)


df_abyss = pd.DataFrame()
for i in range(len(dfs)):
    try:
        df_abyss[f"{i}_abyss"] = dfs[i]["log10_p_abyss_sorted"]
        df_abyss["snps"] = dfs[i]["snps"]
    except Exception as e:
        print(e)

log_columns = [col for col in df_abyss.columns if '_abyss' in col]
mean_log = df_abyss[log_columns].mean(axis=1)
df_abyss = df_abyss[['snps']]
df_abyss["-logP_abyss"] = mean_log
df_abyss.to_pickle(f"../data/iterative_pvalues/pvals_abyss_pheno_{name_risk}_G{G}_L{L}_c{c}_k{k}_M{M}.pkl")