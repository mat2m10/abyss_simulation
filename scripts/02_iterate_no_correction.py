...
for i in range(iterations):    
    ...
    populations['no_corr'] = simulated_case_control
    ...
    for snp in list(complete.columns):
        ...
        try:
            [intercept, coefficients, p_values] = log_reg(populations['no_corr'], geno)
    ...
    df_Ps_nocorr = pd.DataFrame(data={'snps': snps_no_corr, 'P_vals_nocorr': Ps_no_corr})
    log10_p_nocorr = np.sort(-np.log10(df_Ps_nocorr['P_vals_nocorr']))
    df_Ps_nocorr['log10_p_nocorr_sorted'] = log10_p_nocorr
    dfs.append(df_Ps_nocorr)

df_nocorr = pd.DataFrame()
for i in range(len(dfs)):
    try:
        df_nocorr[f"{i}_nocorr"] = dfs[i]["log10_p_nocorr_sorted"]
        df_nocorr[f"{i}_Pvals"] = dfs[i]["P_vals_nocorr"]
        df_nocorr["snps"] = dfs[i]["snps"]
    except Exception as e:
        print(e)

log_columns = [col for col in df_nocorr.columns if '_nocorr' in col]
mean_log = df_nocorr[log_columns].mean(axis=1)
log_columns = [col for col in df_nocorr.columns if 'Pvals' in col]
mean_P = df_nocorr[log_columns].mean(axis=1)
df_nocorr = df_nocorr[['snps']]
df_nocorr["-logP_no_corr"] = mean_log
df_nocorr["P_val"] = mean_P

df_nocorr.to_pickle(f"../data/iterative_pvalues/pvals_nocorr_pheno_{name_risk}_G{G}_L{L}_c{c}_k{k}_M{M}.pkl")