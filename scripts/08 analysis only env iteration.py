...
if len(args) > 1:
    ...
    print("Value parsed from command line:", value)
else:
...
risk_function_no_risk = getattr(helpers, "no_risk_fun")
populations['no_risk'] = list(populations.apply(lambda row: risk_function_no_risk(row['x_temp'], row['y_temp']), axis=1))
...
for i in range(4):
    ...
    simulated_case_control = simulate_cc_status(geno, mu, effect, populations['no_risk'])
    populations['case_control_no_risk'] = simulated_case_control
    populations[[name_risk, 'case_control_env','case_control_no_risk']].to_pickle(f"../data/phenotype/simulatedcase_control_onlyenvrisk_{name_risk}_G{G}_L{L}_c{c}_k{k}_M{M}.pkl")
    ...
    coefs = []
    intercepts = []
    faulty_snps_expected = []
    for snp in list(complete.columns):
        geno = np.array(complete[snp])
        try:
            [intercept, coefficients, p_values] = log_reg(risk['case_control_no_risk'], geno)
            ...
            print(e)
            faulty_snps_expected.append(snp)
    ...
    merged1 = pd.merge(df_Ps_no_corr, df_Ps_PCs, on='snps')
    merged2 = pd.merge(merged1, df_Ps_expected, on='snps')
    # Merge df_Ps_PCs with the already merged dataframe on 'snps'
    final_merged_df = pd.merge(merged2, df_Ps_rare_PCs, on='snps')
    ...
    log10_p_expected = np.sort(-np.log10(final_merged_df['P_vals_expected']))
    ...
    final_merged_df['log10_p_expected_sorted'] = log10_p_expected

    os.makedirs(f"../data/iterative_pvalues", exist_ok=True)
    temp = list(final_merged_df["P_vals_nocorr"])
    
    final_merged_df["P_vals_expected"] = np.sort(final_merged_df["P_vals_expected"])
